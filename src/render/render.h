...
struct accuation { // TODO: Spell this correctly
    long long time_us;
    float acceleration;
    float steering;
...
struct Car {
    ...
    Car(Vect3 setPosition, Vect3 setDimensions, Color setColor, float setVelocity, float setAngle, float setLf, std::string setName)
        : position(setPosition), dimensions(setDimensions), color(setColor), velocity(setVelocity),
          angle(setAngle), Lf(setLf), name(setName) {
